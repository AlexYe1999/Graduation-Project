...
public:
    ...
    void ResizeBuffer(        
        ...
        m_misc      = std::make_unique<Texture2D>(device, DXGI_FORMAT_R32G32B32A32_FLOAT, width, height);
        ...
        // Create RenderTarget View
        {
            ...
            device->CreateRenderTargetView(m_position->GetResource(), &rtvDesc, m_rtvHandles[1]);

            device->CreateRenderTargetView(m_normal->GetResource(), &rtvDesc, m_rtvHandles[2]);

            device->CreateRenderTargetView(m_misc->GetResource(), &rtvDesc, m_rtvHandles[3]);
